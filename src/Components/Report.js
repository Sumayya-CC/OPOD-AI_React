...
const theme = createMuiTheme({
  ...
  class Report extends Component{
      ...
      if(this.state.inputText===''){
        ...
        axios({                         //API Call
          ...
          .then((data)=>{
            ...
            this.setState({lines:this.state.text_tagged.split('.').filter(line =>{
              return /\S/.test(line);   })})  //extracting lines
            var l=(this.state.lines.length-1)? this.state.line_analysis.length: 1;
            var p=Math.floor(l/15);
            this.setState({totalPages: (l%15)? p+1: p});
            ...
            highlighted=this.lineColor(1); //report color code and highlighter
            this.setState({report:true}); //report visible
            // this.pagination(this.state.lines);
            console.log(data)
            console.log(this.state.lines.length)
            console.log(l)
            console.log(this.state.pages)
            var page = document.getElementById("highlight")
            console.log('page'+page.scrollHeight)
    ...
    pagination = (lines) =>{
      console.log(lines)
      this.createPage();
      for (var i = 0; i < lines.length; i++) { // loops through all the words
        var c= this.colorCode(this.state.line_analysis[i].classification, this.state.line_analysis[i].probability*100, 0) //color for lines
        var  b= this.colorCode(this.state.line_analysis[i].classification, this.state.line_analysis[i].probability*100, 1)
          
        var line=this.highlight(lines[i]+'.', this.state.possible_profanity, c, b)
        var success = this.appendToLastPage(line); // tries to fill the word in the last page
        if (!success) { // checks if word could not be filled in last page
            this.createPage(); // create new empty page
            this.appendToLastPage(line); // fill the word in the new last element
        }
    }
    }
    ...
    createPage=() =>{
      ...
      page.innerHTML+=<span>helloo</span>
  ...
  appendToLastPage=(line) =>{
    ...
    page.innerHTML = <span>{page.innerHTML}{line}</span>; // saves the text of the last page
    console.log(page.innerHTML)
    ...
    lineColor = (p) => {
      ...
      if(this.state.lines.length>p*15){
        l=p*15;
      }else{
          ...
          {arr.reduce((prev, current, i) => { 
                  ...
                  return prev.concat(   //add splited array into one line
                      ...
                      //higligting profane words 
                        <span key={i + current} style={{backgroundColor:b, textDecorationLine: 'underline',
                        textDecorationColor:c}}>{current}</span>
                        : current
                    );
              }, [])}
        </span>);
    ...
    addTraining = (range, category) =>{
      this.setState({select:false, range:range, category:category})
        ...
        if(this.state.report){      //Display the report only after clicking the submit button
          ...
          text=(
            <div>
              ...
              <div align='justify' style = {{height:450, width: '96%', marginTop: "7%",  
                ...
                <div style={{whiteSpace:'pre-wrap', marginTop:'1%', marginRight:'1%', marginLeft:'1%', marginBottom:'1%', 
                  height:435, overflow:'hidden',}} 
              ...
              <div className="pages" style={{float:'right', marginRight:'7.5%'}}>
                ...
                <span style = {{marginLeft:'5%',marginRight:'5%',marginTop:'-5%', paddingLeft:'2%', paddingRight:'2%', 
          ...
          report1=(
            ...
            <div>
              <div style={{color:'black', align:"center",marginRight:'33%', marginTop:'-10%',
                display:'inline-flex', float:'right'}}>
                <CloudDownloadIcon style={{color:"black", marginRight:'5%', }}/>
                  Export_Summary
              </div>

              <div style={{ backgroundColor:'#ffffff', height:450, width:'90%',
